...
CI (`.github/workflows/wheels.yml`) builds all platform wheels on one Ubuntu runner by downloading the prebuilt binary from a `lightpanda-io/browser` GitHub release, then tests the installed wheel on native runners. PRs and pushes to `main` build against browser `nightly` and **fail if `lightpanda/_methods.py` is out of date** relative to that binary (fix: regenerate and commit). Publishing is automatic: `track-browser.yml` polls browser releases daily and dispatches a build whose publish job waits for a maintainer's approval on the `pypi` environment (PyPI trusted publishing — no secrets). See README "Releasing".
